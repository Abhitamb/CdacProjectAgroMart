...
function RegCustomer() {
    ...
    const handleSubmit = (event) => {
        ...
        setErrors(uservalidation(customer));
        if (!customer.gender) {
            let obj = { gender: "Gender is required" };
            setErrors(obj);
        }
        setSubmitted(true);
    };
        ...
        if (Object.keys(errors).length === 0 && submitted) {
            axios.post("http://localhost:8081/api/customers", customer)
                .then(resp => {
                    ...
                    sessionStorage.setItem("uname", result.name || ""); // Store uname here
    ...
    }, [errors, submitted, customer, navigate]);
                                ...
                                <div className="form-group form-row">
                                    ...
                                    <div className="col-sm-8">
                                        <input
                                            ...
                                            className="form-control"
                                        />
                                        {errors.cpassword && <small className="text-danger float-right">{errors.cpassword}</small>}
                                    </div>
                                </div>
                                <div className="text-center ml-auto">
                                    <a className="text-white" to="/clogin">Already have an account?</a>
                                </div>
                                <button className="btn btn-primary mt-3 w-100">Register Now</button>
                            </form>
                        </div>
