...
//         axios.get("http://localhost:8081/api/orders?custid="+ customerId)
//         .then(resp=>{
...
//                                     <td><img className="mr-2 float-left" src={"http://localhost:8081/"+x.product.photo} width="100" />
//                                     {x.product.pname}<br/>
...
function MyOrders({ customerId }) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8081/api/orders?custid=${customerId}`)
      .then((resp) => {
        console.log(resp.data);
        setOrders(resp.data.data);
      });
  ...
  const showDetails = (orderId) => {
    axios.get(`http://localhost:8081/api/orders/${orderId}`).then((resp) => {
      console.log(resp.data);
      setDetails(resp.data.data.details);
    });
    setShow(true);
                    ...
                    <tr key={x.product.productId} className="text-center">
                      ...
                      <td>
                        <img
                          ...
                          src={"http://localhost:8081/" + x.product.photo}
                          width="100"
                          alt="Product"
                        />
                        {x.product.pname}
